...
@app.route('/logout')
def do_logout():
    session.pop('type', None) #The session.pop() method removes the specified key from the dictionary and returns its value. If the key does not exist, it returns None    return redirect(url_for('index'))
